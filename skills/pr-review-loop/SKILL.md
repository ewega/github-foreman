...
2. Request a fresh Code Review Agent review.
...
After the review loop is clean and the docs writer task has completed or fallen back locally, poll checks. If checks fail, summarize the failure for the owning agent, request a fix, wait for new commits, then return to the review loop before trying CI again.
...
If the task creates a docs PR, add that PR to wave tracking and include it in review, CI, and the human gate. If the custom-agent task path is unavailable, use the local `docs-writer` subagent and state the fallback.
